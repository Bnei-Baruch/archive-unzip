...
def process_docx_uid(uids):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if any([doc is not None for doc in doc_to_docx_list]):
            ...
            soffice_lock_file = current_app.config['SOFFICE_LOCK_FILE']
            os.makedirs(os.path.dirname(soffice_lock_file),
                        mode=0o777, exist_ok=True)
            docx_list, code, error = doc_to_docx(
                temp_dir, doc_to_docx_list, soffice_bin,
                soffice_lock_file, current_app.logger)
            if code != 200:
